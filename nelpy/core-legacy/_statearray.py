...
def eva_init_wrapper(func):
    ...
    def wrapper(*args, **kwargs):
       ...
       data = kwargs.get('data', [])
        ...
        #and standardizedata to 2D
       data = np.squeeze(data)
        try:
            if(data.shape[0] ==data.size):
               data = np.array(data,ndmin=2)
        except ValueError:
            raise TypeError("Unsupporteddata type!")
...
class EventArray:
    ...
    """
    raise NotImplementedError
    __attributes__ = ['_data','_time', '_support', \
    ...
    def __init__(self,data=[], *, timestamps=None, fs=None,
        ...
        # Note; if we have an empty array ofdata with no dimension,
        # then calling len(data) will return a TypeError
        try:
            # if nodata are given return empty EventArray
            ifdata.size == 0:
        ...
        # Note: if both time anddata are given and dimensionality does not
        # match, then TypeError!

        time = np.squeeze(timestamps).astype(float)
        if(time.shape[0] !=data.shape[1]):
            # self.__init__([],empty=True)
            raise TypeError("time anddata size mismatch! Note:data "
    ...
    defdata(self):
        raise NotImplementedError
        """(np.array N-Dimensional)data that was initially passed in but transposed
        """
        return self._data
    ...
    def isempty(self):
        raise NotImplementedError
        """(bool) checks length ofdata input"""
        try:
    ...
    def _subset(self, idx):
        ...
        eva = self.copy()
        try:
            eva._data = np.atleast_2d(self._data[idx,:])
        except IndexError:
