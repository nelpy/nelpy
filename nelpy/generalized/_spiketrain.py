...
""" idea is to have abscissa and ordinate, and to use aliasing to have n_units,
    _unit_subset, unit_ids, (or trial_ids), and so on.

    What is the genericized class? EventArray? eventseries, eventcollection

    when event <==> spike, abscissa <==> time, eventseries <==> spiketrain
         eventseries_id <==> unit_id, eventseries_type <==> unit, then we have a
         SpikeTrainArray. (n_events, n_spikes)

    unit ==> series (unit, trial, DIO, ...)
