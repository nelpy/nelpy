...
class RinglikeTrajectory(_analogsignalarray.AnalogSignalArray):
    ...
    __attributes__ = [
        "_track_length",
        "_is_wrapped",
    ]  # RinglikeTrajectory-specific attributes
    ...
    def _wrapepochs(self):
        ...
        trial_epochs = _epocharray.EpochArray(
            np.vstack((trial_bds[:-1], trial_bds[1:])).T
        )
    ...
    def asarray(
        ...
        *,
        ...
        if (
            at is None
            and where is None
            and split_by_epoch is False
            and n_points is None
        ):
        ...
        if where is not None:
            assert (
                at is None and n_points is None
            ), "'where', 'at', and 'n_points' cannot be used at the same time"
            if isinstance(where, tuple):
                ...
                assert len(x) == len(
                    self.time
                ), "'where' condition must have same number of elements as self.time"
                at = self.time[x]
        elif at is not None:
            assert (
                n_points is None
            ), "'at' and 'n_points' cannot be used at the same time"
        else:
